...
from .models import Question, Choice, DiscursiveAnswer, MultipleChoiceAnswer, Topic, Test, Discipline
...
class DisciplineList(ListView):
    ...
    def get_queryset(self):
        print "-------SESSION DATA----------"
        print self.request.session.get('teams', {}).get(str(self.request.user.id), {})
...
def subscriber_new(request, template='mydidata/subscriber_new.html'):
...
def my_progress(request):
    ...
    topics = []
    print Discipline.objects.filter(students__id = request.user.id)
            ...
            if request.session.get('teams', False):
                if request.session['teams'].get(str(request.user.id), False):
                    ...
                    for member_id in request.session['teams'][str(request.user.id)]:
                        ...
                        member_answer.save()
                        question.save()
                        print "member answer text"
                        print member_answer.answer_text
                        member_answer = question.answer_set.filter(student=member).first()
                        print "retrieved member answer text"
                        print member_answer.discursiveanswer.answer_text
...
def define_team(request):
    if request.POST:
        ...
        while request.POST.get('selStudent' + str(index), False):
            ...
            print "-------SESSION DATA----------"
            print request.session['teams'][request.user.id]
            index += 1
            request.session.modified = True
        return HttpResponseRedirect(reverse('mydidata:disciplines'))
    else:
        disciplines = Discipline.objects.filter(students__id = request.user.id) 
        print disciplines
        studentsToSelect = [student for discipline in disciplines for student in discipline.students.all()]
        print studentsToSelect
        return render(request, 
